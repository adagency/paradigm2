...
export default class Slide extends Component {
	...
	onEntering = direction => {
		...
		if (this.childRef && this.childRef.onEntering)
			this.childRef.onEntering(direction, timeout, firstAppear);
		else if (
			this.childRef &&
			this.childRef.wrappedInstance &&
			this.childRef.wrappedInstance.onEntering
		)
			this.childRef.wrappedInstance.onEntering(direction, timeout, firstAppear);
	};
	...
	onLeaving = () => {
		...
		if (this.childRef && this.childRef.onLeaving)
			this.childRef.onLeaving(direction, timeout);
		else if (
			this.childRef &&
			this.childRef.wrappedInstance &&
			this.childRef.wrappedInstance.onLeaving
		)
			this.childRef.wrappedInstance.onLeaving(direction, timeout);
	};
